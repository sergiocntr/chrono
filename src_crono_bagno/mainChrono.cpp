...
void setupWifi() {
  WiFi.persistent(false); // Solve possible wifi init errors (re-add at 6.2.1.16
                          // #4044, #4083)
  WiFi.disconnect(true);  // Delete SDK wifi config
  delay(200);
  WiFi.setOutputPower(
      14); // 10dBm == 10mW, 14dBm = 25mW, 17dBm = 50mW, 20dBm = 100mW
  WiFi.mode(WIFI_OFF); // energy saving mode if local WIFI isn't connected
  delay(10);
  WiFi.hostname(
      "bagno"); // DHCP Hostname (useful for finding device for static lease)
  WiFi.mode(WIFI_STA);
  WiFi.forceSleepWake();
  delay(10);
  WiFi.config(ipBagno, gateway, subnet,
              dns1); // Set static IP (2,7s) or 8.6s with DHCP  + 2s on battery
  delay(10);
...
void setup() {
  ...
  // blinkLed(2);
  // sendCrash();
  String clientId = String(chronoId);
...
void checkForUpdates() {
  ...
  // Serial.prconst char* chronoId;intln( fwImageURL );
  yield();
  ...
  if (httpCode == 200) {
    ...
    if (newVersion > versione) {
      ...
      case HTTP_UPDATE_FAILED:
        NtxtOra.setText("U_F");
        // check=1; //Serial.printf("HTTP_UPDATE_FAILD Error (%d): %s",
      ...
      case HTTP_UPDATE_NO_UPDATES:
        ...
        break;
      case HTTP_UPDATE_OK:
        // Serial.println("[update] Update ok."); // may not called we reboot
        // the ESP
        break;
      }
      smartDelay(1000);
...
void getLocalTemp() {
  ...
  smartDelay(100);
  mqttOK = client.publish(casaSensTopic, JSONmessageBuffer,
...
void toggle_alarm() { // ALLARME SI STA PER SPEGNERE L ACQUA LAMPEGGIA PUNTO
                      // ESCLAMATIVO
